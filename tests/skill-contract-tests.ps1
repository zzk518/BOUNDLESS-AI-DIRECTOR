...
Assert-Contains 'Compact AI video visual document' $director @('AI 视频精简视觉交付', '总体影调提示词', '人物、场景、道具资产提示词', '分镜图/关键帧提示词', '4–15 秒视频提示词', '内部分析不铺陈进文档')
Assert-Contains 'Structured prompt format' $director @('媒介、年代与现实程度', '捕获介质、镜头效果、景深', '主色域、辅助色域、强调色及物理来源', '暗部/高光曲线与后期处理', '每条提示词可独立复制')
Assert-Contains 'Responsible style and color controls' $director @('百分比是美术分配意图', '不宣称生成模型能精确执行面积比例', '不用摄影师、导演或具体影片名称代替可见方法', 'Midjourney 图像提示词优先英文', '中文视频模型优先中文')
...
Assert-Contains 'Final master package' $director @('01-锁定剧本.docx', '02-导演视觉方案.docx', '总体影调提示词', '人物/场景/道具资产提示词', '分镜图/关键帧提示词', '完整锁定剧本都不得省略')
